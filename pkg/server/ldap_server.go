...
func (ldapServer *LDAPServer) handleSearch(w ldap.ResponseWriter, m *ldap.Message) {
	...
	gologger.Info().Msgf("[11111111]server端接收到ldap请求：\n%v", message.String())
	...
	//对BaseDn进行切割，如果使用ldap://192.169.85.1/c9tneiitvuoj7i3...这种格式的请求，则baseObject(baseDn)就是c9tneiitvuoj7i3...字符串，所以下面的切割没有意义，不用改了
