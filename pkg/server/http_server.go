...
func (h *HTTPServer) logger(handler http.Handler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		req, _ := httputil.DumpRequest(r, true) //DumpRequest方法是将request请求格式化输出
		reqString := string(req)

		gologger.Info().Msgf("[hd测试]server端接收到Http请求:\n%s", reqString)
		rec := httptest.NewRecorder() //rec是初始化后的响应数据
		handler.ServeHTTP(rec, r)     //将初始化后的rec响应数据和r请求绑定
		...
		if h.options.RootTLD && strings.HasSuffix(r.Host, h.domain) { //RootTLD默认关闭：enable wildcard interaction for interactsh domain (authenticated)(为interactsh域(已验证)启用通配符交互  )
			ID := h.domain
		...
		if h.options.ScanEverywhere {
			...
			//这里是按照域名的方式对子域名进行解析，分析是否是唯一标识，这种方式只是能识别客户端是哪个，而不能判断是哪个漏洞发送的这个log
			/*parts := strings.Split(r.Host, ".")
			for i, part := range parts {
				for partChunk := range stringsutil.SlideWithLength(part, h.options.GetIdLength()) {
					if h.options.isCorrelationID(partChunk) {   //partChunk是子域名中的CorrelationID,子域名由correlationID+randomData组成
						fullID := part              //fullID是客户端生成的子域名
						if i+1 <= len(parts) {
			...
			//jackey0新增，不能按照域名的方式进行解析了，要去解析url请求，需要设计漏洞存在的话接受的唯一标识
...
func (h *HTTPServer) pollHandler(w http.ResponseWriter, req *http.Request) {
	...
	data, aesKey, err := h.options.Storage.GetInteractions(ID, secret) //将server端存储的dnslog日志信息拿出来
	if err != nil {
